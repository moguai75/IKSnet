...
namespace IKSnet.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            // Status Benutzer abfragen
            ApplicationDbContext db = new ApplicationDbContext();
            var login = db.Users.SingleOrDefault(l => l.UserName == model.Email)?.Status;
            // Wenn Status gesperrt, Login verweigern
            if (login == Status.Deaktiviert)
            {
                ModelState.AddModelError("", "Benutzer hat Status deaktiviert.");
                return View(model);
            }
            // Wenn Status erfasst, Login verweigern
            else if (login == Status.Erfasst)
            {
